...
class HumanizerExtension extends AbstractExtension
{
    ...
    {
        if ($time instanceof \DateTimeInterface) $time = $time->getTimestamp();
    ...
    public static function secondsInTime($time): string
    {
        ...
        $time = floatval($time);
        if ($time < 5) return number_format($time, 1, ',').' s';
        if ($time < 70) return sprintf('%d s', $time);
        if ($time < 3700) return sprintf('%d min %02d s', $time/60, $time%60);
        if ($time < 90000) return sprintf('%d h %02d min %02d s', $time/3600, ($time%3600)/60, ($time%3600)%60);
        else return sprintf('%d dia %02d h %02d min', $time/86400, ($time%86400)/3600, (($time%86400)%3600)/60);
    }

    public static function ellipsis($value, $window=8): string
    {
        if (empty($value) || strlen($value) < 20) return $value;
